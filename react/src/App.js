...
// for useEffect see https://blog.logrocket.com/modern-api-data-fetching-methods-react/
 useEffect(() => {
  ...
  return (
    ...
    <tr key={index}>
            <td><img src={`https://avatars.githubusercontent.com/${item.github}?v=4`} width="21" alt="GitHub profile"></img></td>
            <td><a href={`https://github.com/${item.github}`}>{`${item.github}`}</a></td>
            <td><a href={`http://${item.hostname}`}>{`${item.hostname}`}</a></td>
            <td><a href={`https://view.matrix.org/room/!AmypvmJtUjBesRrnLM:matrix.org/members/${item.matrix}`}>{item.matrix.split(':')[0]}</a></td>
            <td>{item.tz}</td>
    </tr>
          ))}
      </tbody>
